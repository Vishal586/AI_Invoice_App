...
const AllInvoices = () => {
  ...
  if (loading) {
    return <div className="flex justify-center w-8 h-8 animate-spin text-blue-600"><Loader2 className="" /></div>
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button onClick={() => navigate('/invoices/new')} icon={Plus}>Create Invoices</Button>
        </div>
      </div>
        ...
        <div className="p-4 sm:p-6 border-b border-slate-200">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="w-full sm:w-auto">
              <select
                className="w-full sm:w-48 h-10 px-3 py-2 border border-slate-200 rounded-lg bg-white text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500"
                value={statusFilter}
        ...
        {filteredInvoices.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            {invoices.length === 0 && (
              ...
              } icon={Plus}>Create First Invoice</Button>
            )}
          </div>
        ) : (
              ...
              <tbody className="bg-white divide-y divide-slate-200">
                {filteredInvoices.map(invoice => (
                  <tr key={invoice._id} className="hover:bg-slate-50">
                    ...
                    <td onClick={() => navigate(`/invoices/${invoice._id}`)} className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 cursor-pointer">{moment(invoice.dueDate).format('MM DD YYYY')}</td>

...
const InvoiceDetail = () => {
  ...
  useEffect(() => {
    const fetchInvoice = async () => {
      try {
        // FIX: Check if API_PATHS and its properties exist
        let apiPath;
        if (API_PATHS && API_PATHS.INVOICES && API_PATHS.INVOICES.GET_INVOICE_BY_ID) {
          apiPath = API_PATHS.INVOICES.GET_INVOICE_BY_ID(id);
        } else if (API_PATHS && typeof API_PATHS.GET_INVOICE_BY_ID === 'function') {
          // Alternative: API_PATHS might have the function directly
          apiPath = API_PATHS.GET_INVOICE_BY_ID(id);
        } else {
          // Fallback to the standard API endpoint
          apiPath = `/api/invoices/${id}`;
        }
      ...
      <div id="invoice-content-wrapper">
        <div ref={invoiceRef} id="invoice-preview" className="bg-white p-6 sm:p-8 md:p-12 rounded-lg shadow-md border border-slate-200 pb-[100vh]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 my-8">
            <div>
              <h3 className="text-sm font-semibold text-slate-500 upparcase tracking-wider mb-3">Bill From</h3>
            ...
            <div className="sm:text-right">
              <h3 className="text-sm font-semibold text-slate-500 upparcase tracking-wider mb-3">Bill To</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 my-8">
            ...
            <div>
              <h3 className="text-center text-sm font-semibold text-slate-500 uppercase tracking-wider mb-3">Due Date</h3>
              <p className="text-center font-medium text-slate-800">{new Date(invoice.dueDate).toLocaleDateString()}</p>
            </div>

            <div>
              <h3 className="text-right text-sm font-semibold text-slate-500 uppercase tracking-wider mb-3">Payment Terms</h3>
              <p className="text-right font-medium text-slate-800">{invoice.paymentTerms}</p>
            </div>
          </div>
